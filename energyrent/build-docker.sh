...
# 🚀 Energy Rental Service 统一构建脚本
# 功能: NestJS 应用 Docker 镜像构建

set -e

echo "🚀 Enclave Energy Rental Service 构建开始"
echo "=========================================="
...
# 默认配置
IMAGE_NAME="aigen2025/enclave-energyrent"
VERSION="${VERSION:-v1}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  $0 --tag myrepo/energyrent --version v1 # 构建 myrepo/energyrent:v1"
            echo "  $0 --push                            # 构建并推送镜像"
...
if [ $? -eq 0 ]; then
    ...
    if [ "$PUSH_IMAGE" != true ]; then
        ...
        echo ""
        echo -e "${BLUE}💡 使用方法:${NC}"
        echo -e "   docker run -p 4001:4001 ${FULL_IMAGE_TAG}"
