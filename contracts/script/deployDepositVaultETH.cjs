...
// Ethereum Mainnet AAVE V3 Pool
const AAVE_POOL_ETHEREUM = "0x87870Bca3F3fD6335C3F4ce8392D69350B4fA4E2";

/**
 * @title Deploy DepositVault on Ethereum
 * @dev 在 Ethereum 主网部署 DepositVault
 * 
 * 使用方法:
 ...
 *   - RPC_URL: Ethereum RPC URL（可选，默认使用公共节点）
 *   - AAVE_DELEGATE: AAVE V3 适配器地址（可选，如果不提供会自动部署）
 *   - AAVE_POOL: AAVE V3 Pool 地址（可选，默认使用 Ethereum 主网地址）
 */
async function main() {
    console.log("====================================");
    console.log("Deploying DepositVault on Ethereum");
    console.log("====================================");
    ...
    // 2. 获取 AAVE Pool 地址
    const aavePool = process.env.AAVE_POOL || AAVE_POOL_ETHEREUM;
    ...
    const outputPath = path.join(rootDir, "deployed", "result_ethereum.json");
