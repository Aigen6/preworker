...
// Use system fonts instead of Google Fonts to avoid network issues during Docker build
...
export const metadata: Metadata = {
  ...
  description: appConfig.brandSubtitle,
  icons: {
    icon: appConfig.favicon,
    shortcut: appConfig.favicon,
    apple: appConfig.favicon,
  },
}

// 防止移动端输入框聚焦时自动放大
// 通过限制最大缩放为 1 来防止 iOS Safari 自动放大页面
export const viewport: Viewport = {
  width: 'device-width',
  initialScale: 1,
  maximumScale: 1,        // 限制最大缩放为 1，防止自动放大
  userScalable: false,    // 禁止用户手动缩放（如果需要可改为 true）
}
