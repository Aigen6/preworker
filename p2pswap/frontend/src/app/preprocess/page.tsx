...
import { estimateApproveEnergy, estimateApproveBandwidth } from "@/lib/utils/tron-energy-estimator"
import { normalizeTronAddress } from "@/lib/utils/tron-address-converter"
import "@/lib/utils/debug-tron-energy" // 在开发环境自动调试
...
function PreprocessPage() {
  ...
  // 当钱包连接时，默认使用当前用户地址作为 intendedRecipient（仅在首次连接时设置）
  useEffect(() => {
    if (address && intendedRecipient === "") {
      setIntendedRecipient(address)
    }
  }, [address]) // 移除 intendedRecipient 依赖，只在 address 变化时设置默认值
  ...
  const getUSDTAddress = useCallback((chainId: number): string | null => {
    const USDT_ADDRESSES: Record<number, string> = {
      ...
      195: 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t', // TRON (SLIP-44)
    }
  ...
  const handleDeposit = async () => {
    ...
    if (!address) {
      showWarning('无法获取钱包地址，请重新连接钱包')
      return
    }
    if (!chainId) {
      showWarning('无法获取链 ID，请确保钱包已正确连接到 TRON 网络')
      return
    }
    if (!vaultAddress) {
      showWarning('合约地址加载中，请稍候...')
      return
    }
  ...
  const handleQueryClaimableDeposits = useCallback(async () => {
    ...
    try {
      ...
      for (const item of depositsList) {
        try {
          // 验证 depositId 是否有效
          if (!item.depositId || item.depositId.trim() === '' || item.depositId === 'undefined' || item.depositId === 'null') {
            console.warn(`跳过无效的 depositId: "${item.depositId}"`)
            continue
          }
          ...
          // 地址比较：对于 TRON 链，地址可能是 Base58 或 hex 格式，需要统一比较
          // TRON 地址比较应该不区分大小写
          const normalizeAddressForComparison = (addr: string, chainId: number): string => {
            if (!addr) return ''
            
            // 对于非 TRON 链，直接比较（不区分大小写）
            if (chainId !== 195) {
              return addr.toLowerCase()
            }
            
            // 对于 TRON 链，使用统一的地址转换工具
            try {
              const base58Addr = normalizeTronAddress(addr)
              // TRON Base58 地址比较不区分大小写
              return base58Addr.toLowerCase()
            } catch (err) {
              console.warn('地址转换失败，使用原始值比较:', err)
              // 如果转换失败，回退到简单的字符串比较（不区分大小写）
              return addr.toLowerCase()
            }
          }
          
          // 比较地址（同步函数）
          const normalizedRecipient = normalizeAddressForComparison(info.intendedRecipient, chainId)
          const normalizedAddress = normalizeAddressForComparison(address, chainId)
          const addressMatch = normalizedRecipient === normalizedAddress
          
          console.log(`  - intendedRecipient (normalized): ${normalizedRecipient}`)
          console.log(`  - 当前地址 (normalized): ${normalizedAddress}`)
          console.log(`  - 地址匹配: ${addressMatch}`)
          console.log(`  - yieldAmount: ${info.yieldAmount}`)
          
          // 只添加未使用的存款
          if (info && !info.used && addressMatch) {
            ...
            if (!info) {
              ...
              try {
                ...
                if (normalizedRecipient !== normalizedAddress) {
                  ...
                  console.warn(`    intendedRecipient: "${info.intendedRecipient}" (normalized: "${normalizedRecipient}")`)
                  console.warn(`    当前地址: "${address}" (normalized: "${normalizedAddress}")`)
                  
                  // 判断地址格式
                  const getAddressFormat = (addr: string): string => {
                    if (!addr) return 'Empty'
                    if (addr.startsWith('T') || (addr.length === 34 && !addr.startsWith('0x') && !addr.startsWith('41'))) {
                      return 'Base58'
                    }
                    if (addr.startsWith('0x') || (addr.startsWith('41') && addr.length === 42)) {
                      return 'Hex'
                    }
                    return 'Unknown'
                  }
                  
                  console.warn(`    地址格式: intendedRecipient=${getAddressFormat(info.intendedRecipient)}, address=${getAddressFormat(address)}`)
                }
              } catch (err) {
                console.warn(`  - 原因: intendedRecipient 不匹配（地址转换失败）`)
                console.warn(`    intendedRecipient: "${info.intendedRecipient}"`)
                console.warn(`    当前地址: "${address}"`)
                console.warn(`    转换错误:`, err)
              }
            }
          }
        } catch (error) {
        ...
        <div className="bg-black-2 rounded-[12px] p-6">
          ...
          <div className="mb-4">
            ...
            <AddressInput
              ...
              chainId={chainId}
              showValidation={true}
              placeholder={address ? `默认: ${address.slice(0, 6)}...${address.slice(-4)}` : undefined}
            />
            {!isRecipientValid && intendedRecipient && (
              <p className="text-xs text-red-500 mt-2">
                地址格式无效，请根据当前链（{chainId === 195 ? 'TRON' : chainId === 60 || chainId === 1 ? 'Ethereum' : chainId === 714 || chainId === 56 ? 'BSC' : `Chain ${chainId}`}）输入正确的地址格式
              </p>
            )}
          </div>
          ...
          <div className="mb-4">
            {(() => {
              // 计算当前状态和能量需求
              const currentOperationType = !isAuthorized ? 'approve' : 'justlending-supply'
              const calculatedEnergy = chainId === 195 
                ? (!isAuthorized 
                    ? estimateApproveEnergy(getUSDTAddress(chainId), parseUSDTAmount(depositAmount, chainId))
                    : undefined
                  )
                : undefined
              const calculatedBandwidth = chainId === 195 
                ? (!isAuthorized 
                    ? estimateApproveBandwidth()
                    : undefined
                  )
                : undefined
              
              // 打印日志
              console.log('=== 存入标签页 - TRON Gas 配置 ===')
              console.log('当前状态:', {
                isAuthorized,
                chainId,
                operationType: currentOperationType,
              })
              console.log('计算的 Energy:', calculatedEnergy)
              console.log('计算的 Bandwidth:', calculatedBandwidth)
              console.log('使用的配置:', {
                operationType: currentOperationType,
                requiredEnergy: calculatedEnergy || '使用 operationType 配置值',
                requiredBandwidth: calculatedBandwidth || '使用 operationType 配置值',
              })
              console.log('====================================')
              ...
              return (
                <TronGasRentalOption
                  operationType={currentOperationType}
                  // 授权阶段：使用 APPROVE Energy 配置
                  // 存入阶段：使用 JUSTLENDING_SUPPLY Energy 配置
                  requiredEnergy={calculatedEnergy}
                  requiredBandwidth={calculatedBandwidth}
      ...
      {activeTab === "claim" && (
        <div className="bg-black-2 rounded-[12px] p-6">
          ...
          <div className="mb-4">
            {(() => {
              // 打印日志
              console.log('=== 领取标签页 - TRON Gas 配置 ===')
              console.log('当前状态:', {
                activeTab: 'claim',
                operationType: 'justlending-withdraw',
                chainId,
              })
              console.log('使用的配置:', {
                operationType: 'justlending-withdraw',
                requiredEnergy: '使用 operationType 配置值',
                requiredBandwidth: '使用 operationType 配置值',
              })
              console.log('====================================')
              ...
              return (
                <TronGasRentalOption
                  operationType="justlending-withdraw"
                  // 提取/赎回操作：使用 NEXT_PUBLIC_TRON_ENERGY_JUSTLENDING_WITHDRAW_ENERGY
                  // 不传 onRentClick，使用组件内置的直接租赁功能
                />
              )
            })()}
          ...
          <button
            ...
            className="w-full mb-4 h-12 rounded-xl font-medium text-sm text-white bg-black-3 border border-black-4 hover:bg-black-4 hover:border-primary/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group"
          >
            {isLoadingClaimable ? (
              <>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {claimableDeposits.length > 0 ? (
                claimableDeposits.map((item, index) => {
                  ...
                  // 注意：合约返回的是链上实际精度的值（TRON/ETH 是 6 位，BSC 是 18 位）
                  const usdtDecimals = getUSDTDecimals(chainId ?? 195)
                  const usdtAmount = formatFromWei(item.underlyingAmount, usdtDecimals)
          ...
          <div className="mb-4">
            {(() => {
              ...
              return (
                <TronGasRentalOption
                  operationType="justlending-withdraw"
                  // 退回操作（recover）：也是提取操作，使用 NEXT_PUBLIC_TRON_ENERGY_JUSTLENDING_WITHDRAW_ENERGY
                  // 不传 onRentClick，使用组件内置的直接租赁功能
                />
              )
            })()}
        ...
        <div className="p-4">
          ...
          {txStatus === 'failed' && (
            <>
              ...
              <h2 className="text-xl font-medium text-danger mb-4 text-center">交易失败</h2>
              <p className="text-black-9 text-center mb-4">
                交易执行失败，请重试
              </p>
            </>
