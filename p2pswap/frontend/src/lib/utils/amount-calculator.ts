...
export function formatFromWei(weiAmount: bigint | string, decimals: number = 18): string {
  // 防御性检查：如果传入的是数组，取第一个元素
  if (Array.isArray(weiAmount)) {
    console.warn('[formatFromWei] 收到数组，取第一个元素:', weiAmount)
    if (weiAmount.length === 0) {
      return formatUnits(0n, decimals)
    }
  ...
  if (weiAmount === null || weiAmount === undefined) {
    console.warn('[formatFromWei] 收到 null/undefined，使用 0')
    return formatUnits(0n, decimals)
  }
  ...
  } else if (weiAmount && typeof weiAmount.toString === 'function') {
    // 可能是 BigNumber 对象
    if ((weiAmount as any)._hex) {
      value = BigInt((weiAmount as any)._hex)
    } else {
      value = BigInt(weiAmount.toString())
    }
  } else {
    console.warn('[formatFromWei] 无法解析值，使用 0:', weiAmount)
    return formatUnits(0n, decimals)
  }
