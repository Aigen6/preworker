...
import { readTronTokenBalance } from '../utils/tron-rpc-reader'
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          // 对于 TRON 链，使用自定义 RPC 查询余额，避免使用钱包的默认 RPC（可能限流）
          if (chainId === TRON_CHAIN_ID || account.chainType === 'tron') {
            try {
              console.log('[useWalletBalance] 使用自定义 RPC 查询 TRON 余额')
              balanceResult = await readTronTokenBalance(actualUsdtAddress, account.nativeAddress)
              console.log('[useWalletBalance] ✅ 使用自定义 RPC 查询 TRON 余额成功')
            } catch (tronErr) {
              console.warn('[useWalletBalance] 自定义 RPC 查询失败，回退到 walletManager:', tronErr)
              // 回退到使用 walletManager（可能会遇到 429 错误）
          ...
          else if (account.chainType === 'evm' && typeof window !== 'undefined' && window.ethereum) {
            try {
              // 使用 eth_call 直接调用合约
